...
d %>% filter(dec_o == 1) %>%
  group_by(wave) %>%
  ggplot(aes(x = factor(dec_o), fill = gender)) + geom_bar(position = "fill") + facet_grid(.~ wave)
...
t1 %>% ggplot(aes(x = wave, fill = gender)) + geom_bar()+xlab("Session") 
...
```{r fig.cap = "Distribution of gender by session", fig.width=5, fig.height=2.5, echo=FALSE, fig.pos='h',out.height = "2.5in",fig.align='center',fig.pos = 'H'}
t1 %>% ggplot(aes(x = wave, fill = gender)) + geom_bar(position = "fill") #Distribution of gender by wave
```
...
t2 %>% ggplot(aes(x = factor(wave), y = `percentage of yes`, 
  ...
  geom_point() + geom_line() + ylim(c(0, 70))+ xlab("Session")
```
 ...
 As it was discussed in the descriptive analysis, participants in the study were from different nationalities. Since the majority of them were from the US, it is worth considering what percentage of American or international participants were successful to find someone to date with. 
...
```
 Based on the plot, there was not a big difference in the percentage of match among US or international students. Around 15 % of the international participants could find some matches in the whole study, and this rate is just a little more for the American students.
